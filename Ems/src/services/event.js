...
export const updateEvent = async(data, token)=>{
    console.log(token)
    try {
        const res = await apiConnector("PUT",`${eventEndpoints.UPDATE_EVENT_API}/${data.eventId}`,data,
            {authorization: `Bearer ${token}`}
        )
        console.log(res.data)
        console.log(res.data)
        if(!res.data.success){
            toast.error(res.data.message || "Couldn't update an event");
            throw new Error("Error updating an event")
        }
        ...
        toast.error(error?.response?.data?.message || error.message || "Something went wrong while updating the event");
    }
